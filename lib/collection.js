...
class Collection {
  ...
  upsert(record) {
    ...
    const updateOp = {$set: {}, $setOnInsert: {_id: id}}
    for(let k in record) {
      if(k in updateOp && isPlainObject(updateOp[k])) {
        Object.assign(updateOp[k], record[k])
      } else if(k[0] === '$') {
        updateOp[k] = record[k]
      } else {
        updateOp.$set[k] = record[k]
      }
    }
    return this.coll.findOneAndUpdate(this._getQuery(record), updateOp, {
      projection: {_id: 1},
      upsert: true
    })
      .then(updateResult => {
  ...
  update(record, identifier) {
    ...
    //   record, identifier
    // })
    let update = { $set: record };
    // allow passing
    if('$set' in record || '$push' in record || '$unset' in record) {
      update = record;
    }
    return this.coll.updateMany(identifier, update)
  }
  ...
  remove(selector) {
    return this.coll.removeOne(selector)
  }
